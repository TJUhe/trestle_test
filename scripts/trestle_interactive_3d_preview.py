...
<body>
    ...
    <aside>
      ...
      <section class="analysis-panel" aria-label="结构受力分析">
        ...
        <div class="input-grid">
          ...
          <div class="field">
            <label for="deformScale"><span>变形放大倍数</span><b id="deformScaleValue">60×</b></label>
            <input id="deformScale" type="range" min="0" max="200" step="5" value="60" autocomplete="off">
          </div>
        </div>
  ...
  <script type="module">
    ...
    function displayBoostAtStation(xMm, loadSummary) {{
      ...
      return maxShown * 0.09 * Math.sqrt(Math.min(1, span.span / controllingSpan)) * spanShape;
    }}
    ...
    function makeTextSprite(text, options = {{}}) {{
      ...
      context.fillStyle = options.background || 'rgba(255, 255, 255, 0.62)';
      context.strokeStyle = options.border || 'rgba(41, 53, 65, 0.18)';
      ...
      const material = new THREE.SpriteMaterial({{ map: texture, transparent: true, opacity: options.opacity || 0.82, depthTest: false }});
    ...
    function makeLabelAnchor(xMm, yMm, zMm, text, options = {{}}) {{
      ...
      const marker = new THREE.Mesh(new THREE.SphereGeometry(0.16, 16, 12), new THREE.MeshStandardMaterial({{ color, transparent: true, opacity: options.markerOpacity || 0.62, roughness: 0.45 }}));
      marker.position.set(x, y, z);
      const line = new THREE.Line(
        new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(x, y, z), new THREE.Vector3(x, y, z + verticalOffset)]),
        new THREE.LineBasicMaterial({{ color, transparent: true, opacity: options.lineOpacity || 0.46 }})
      );
    ...
    function rebuildDeformationLabels(loadSummary) {{
      ...
      for (const reaction of loadSummary.reactions) {{
        ...
        labelGroup.add(makeLabelAnchor(
          ...
          {{ color: '#0f6b46', background: 'rgba(232, 247, 239, 0.56)', border: 'rgba(15, 107, 70, 0.18)', colorValue: 0x15804f, fontSize: 30, offset: 1.35, opacity: 0.78 }}
        ));
      }}
      for (const span of loadSummary.spans) {{
        ...
        const zMm = deckZAt(xMm) - deflectionAtStation(xMm, loadSummary) + 1850;
        labelGroup.add(makeLabelAnchor(
          xMm,
          -labelY,
          zMm,
          `Z${{span.index + 1}}-Z${{span.index + 2}} ${{formatNumber(actualMm, 3)}} mm`,
